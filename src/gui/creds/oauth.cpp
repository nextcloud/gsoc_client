        ...
        while (QTcpSocket *socket = _server.nextPendingConnection()) {
            ...
            QObject::connect(socket, &QIODevice::readyRead, this, [this, socket]{
                ...
                QObject::connect(reply, &QNetworkReply::finished, this, [this,reply] {
                    ...
                    reply->deleteLater();
                    bool ok = false;
                    QVariant var = QtJson::parse(json, ok);
                    QString accessToken = var.toMap()["access_token"].toString();
                    QString refreshToken = var.toMap()["refresh_token"].toString();
                    QString user = var.toMap()["user_id"].toString();

                    if (reply->error() != QNetworkReply::NoError || !ok || user.isEmpty()
                            || refreshToken.isEmpty() || accessToken.isEmpty()
                            || var.toMap()["token_type"] != QLatin1String("Bearer")) {
                        qDebug() << "Error when getting the accessToken" << reply->error() << json;
                        emit result(Error);
                        return;
                    }
